...
import {getPushToken, getUser} from "../redux/actions/user-action";
...
const HomeScreen = (props) => {
    ...
    const {
        userData: {
            member: {Amount,Country, Phone, InvestedAmount, ReferralID, ID, LastName},
            bankDetails,
        },
    } = user;
    const {allProjects, loadingProject} = data;
    useEffect(() => {
        dispatch(getAllProject());
    }, []);

    useEffect(() => {
        const userGuideTimer = setTimeout( () =>{
            dispatch(toggleUserGuide());
        }, 3600)

        return () => clearTimeout(userGuideTimer);
    }, []);
    ...
    const sendPushToken = useCallback((token) => {
        const pushDetails = new FormData();

        pushDetails.append('userId', ID)
        pushDetails.append('pushToken', token)
        dispatch(getPushToken(pushDetails))
    ...
    return (
        <>
            ...
            <AnimatedScrollView
                ...
                routeMessage={`${LastName} Welcome Home`}
                routeName="Dashboard"
            >
                <View style={styles.container}>
                            ...
                            >
                                <Pressable
                                    style={[
                                        {
                                            backgroundColor:
                                                theme === "Dark" ? DarkColors.primaryDarkThree : "#ddd",
                                        },
                                        styles.buttonClose,
                                    ]}
                                ...
                                <View style={styles.modalContent}>
                                    ...
                                    >
                                        Get ₦1,000 when you refer a friend to invest and another
                                        N1500 when the person you referred refers another person to
                                        invest, copy and share your referral link.
                                    </Text>
                    ...
                    >
                        <View style={styles.refTextWrap}>
                            <Text style={styles.refTitle}>Refer and earn</Text>
                    ...
                    <View
                        style={[theme === "Dark" ? styles.buttonDeckD : styles.buttonDeckW]}
                    >
                        <ImageBackground
                            style={{
                                width: "100%",
